    ...
    def fill(self, paf, stride, min_distance=0.0, max_distance=None):
        ...
        for paf_i, fourds in enumerate(paf):
            ...
            if self.pif_floor < 1.0:
                ...
                scores_b = scores
            mask_b = scores_b > self.score_th
            scores_b_masked = np.expand_dims(np.expand_dims(scores_b[mask_b], 0), 0)
            d7_b = np.concatenate((
                np.repeat(scores_b_masked, 2, axis=0),
                fourds[::-1, 1:][:, :, mask_b],
            ), axis=1)
            if self.backward[paf_i] is None:
                self.backward[paf_i] = d7_b
            else:
                self.backward[paf_i] = np.concatenate((self.backward[paf_i], d7_b), axis=2)
            ...
            if self.pif_floor < 1.0:
                ...
                scores_f = scores
            mask_f = scores_f > self.score_th
            scores_f_masked = np.expand_dims(np.expand_dims(scores_f[mask_f], 0), 0)
            d7_f = np.concatenate((
                np.repeat(scores_f_masked, 2, axis=0),
                fourds[:, 1:][:, :, mask_f],
            ), axis=1)
            if self.forward[paf_i] is None:
                self.forward[paf_i] = d7_f
            else:
                self.forward[paf_i] = np.concatenate((self.forward[paf_i], d7_f), axis=2)

        LOG.debug('scored paf (%d, %d) in %.3fs',
                  sum(f.shape[2] for f in self.forward),
                  sum(b.shape[2] for b in self.backward),
                  time.perf_counter() - start)
        return self
