    ...
    def __call__(self, fields, initial_annotations=None, debug=None):
        ...
        with torch.autograd.profiler.profile() as prof:
            ...
            if cif is None:         # decode only segmentation
                ...
                # with open('runtime_decoder_cifpan_log.txt','a') as f:
                #     f.write('-------------------------------------------------------')
                #     f.write('-------------------------------------------------------')
            ...
            if self.args.use_panoptic_deeplab_output_decode:
                ...
                pan_r_image = Image.open('/home/ucl/elen/abolfazl/panopticdeeplabsport/imag/output_infer/panoptic/panoptic_pred_'+str(self.numb)+'.png')
                # print('image size 0',pan_r_image.size)
                pan_image = np.array(pan_r_image)
                # print('image size 1',pan_image.shape)
                if pan_image.dtype == np.uint8:
                    pan_image = pan_image.astype(np.int32)
                pan_instances = pan_image[:, :, 0] + 256 * pan_image[:, :, 1] + 256 * 256 * pan_image[:, :, 2]
                # print('image size',pan_instances.shape)
                # print('uniques', np.unique(pan_instances))
                # instances = pan_instances
                p_uniques = np.unique(pan_instances)
                
                
                # print('unique 1', p_uniques)
                # instances_bmask = []
                centers_fyxv_temp = []
                for ix, u in enumerate(p_uniques):
                    instances_bmask = np.where(pan_instances == u, 1, 0)
                    pan_instances = np.where(pan_instances == u, ix, pan_instances)
                    
                    meshgrid = np.indices(instances_bmask.shape)
                    meshgrid[0] *= instances_bmask
                    meshgrid[1] *= instances_bmask
                    center = (meshgrid[0].sum()/instances_bmask.sum(),
                            meshgrid[1].sum()/instances_bmask.sum())
                    centers_fyxv_temp.append((Ci,center[1], center[0], 1))
                # print('unique 2', np.unique(pan_instances))
                instances = pan_instances
                semantic = np.zeros((2,instances.shape[0],instances.shape[1]))
                semantic[0,:,:] = np.where(instances==0,1,0)
                semantic[1,:,:] = np.where(instances>0,1,0)
            ...
            if self.args.use_panoptic_deeplab_output_decode:    ## to make use of centers from panoptic deeplab
                centers_fyxv = centers_fyxv_temp
            ...
            skipped_keypoints = []
            # print('cifhr shape', cifhr.accumulated[0].shape)
            if self.decode_masks_first:
                for i in range(len(annotations)):
