...
class CropKeemotion(Preprocess):
    ...
    def area_of_interest(anns, valid_area, edge_length):
        ...
        if len(anns[0]['keypoints']) == 1:      # for ball
            anns_of_interest = [
                ann['keypoints'][0] for ann in anns
            ]
        else:       # for instances
            anns_of_interest = [
                ann['keypoints'][17] for ann in anns
            ]
        if anns_of_interest == []:
            return valid_area
        
        rand_inst = random.randrange(len(anns_of_interest))

        edge_length / 2
        min_x = min(anns_of_interest[rand_inst][0] - edge_length / 2, valid_area[0])
        min_y = min(anns_of_interest[rand_inst][1] - edge_length / 2, valid_area[1])
