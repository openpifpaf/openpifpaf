...
class VisualGenome(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, *,
        ...
        self.graphs_file = os.path.join(self.data_dir, "VG-SGG.h5")
        ...
        self.ind_to_predicates = sorted(self.predicate_to_ind, key=lambda k: self.predicate_to_ind[k])
        # cfg.ind_to_predicate = self.ind_to_predicates

        self.split_mask, self.image_index, self.im_sizes, self.gt_boxes, \
            self.gt_classes, self.relationships, self.image_ids = self.load_graphs(
                num_im, num_val_im=num_val_im,
    ...
    def _im_getter(self, idx):
        ...
        im = self.im_refs[self.image_index[idx]]
        im = im[:, :h, :w]  # crop out
    ...
    def __getitem__(self, index):
        ...
        local_file_path = os.path.join(self.data_dir, 'VG_100K', f'{image_id}.jpg')
        meta = {
            'dataset_index': index,
            'image_id': image_id,
            'file_name': image_id,
    ...
    def load_graphs(self, num_im=-1, num_val_im=0, filter_empty_rels=True,
                    filter_non_overlap=False):
        """
        Load the file containing the GT boxes and relations, as well as the dataset split

        self.graphs_file: HDF5
        self.split: (train, val, or test)
        ...
        roi_h5 = h5py.File(self.graphs_file, 'r')
        im_h5 = h5py.File(self.image_file, 'r')

        data_split = roi_h5['split'][:]
        split = 2 if self.split == 'test' else 0
        ...
        split_mask = np.zeros_like(data_split, dtype=bool)
        split_mask[image_index] = True

        # Get box information
        all_labels = roi_h5['labels'][:, 0]
        all_boxes = roi_h5['boxes_{}'.format(512 if self.use_512 else 1024)][:]  # will index later
