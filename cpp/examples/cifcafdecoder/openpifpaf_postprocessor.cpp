...
static const float seedThreshold = 0.2f;
static const float keypointThreshold = 0.001f;
...
OpenPifPafPostprocessor::Paf_target
  OpenPifPafPostprocessor::scorePafTarget(const std::vector<float>& paf,
                                          ...
                                          float score_th)
{
  ...
  for (int c = 0; c < 19; ++c) {
    ...
    std::vector<int> mask;
    for (int i = 0; i < H * W; ++i) {
      auto a = paf[pafOffset + i];
      auto b = paf[pafOffset + paf_stride_1 + i];
    ...
    for (size_t i = 0; i < mask.size(); ++i) {
      const auto m = mask[i];
      masked[i                ] = paf[pafOffset                                 + m];
      masked[i + scores_size  ] = paf[pafOffset                + paf_stride_2   + m];
      masked[i + scores_size*2] = paf[pafOffset                + paf_stride_2*2 + m];
      masked[i + scores_size*3] = paf[pafOffset                + paf_stride_2*3 + m];
      masked[i + scores_size*4] = paf[pafOffset + paf_stride_1                  + m];
      masked[i + scores_size*5] = paf[pafOffset + paf_stride_1 + paf_stride_2   + m];
      masked[i + scores_size*6] = paf[pafOffset + paf_stride_1 + paf_stride_2*2 + m];
      masked[i + scores_size*7] = paf[pafOffset + paf_stride_1 + paf_stride_2*3 + m];
    }
                                      ...
                                      const std::vector<float>& pifhrCore)
{
  ...
  for (int field_i = 0; field_i < 17; ++field_i) {
    ...
    for (auto c : sorted) {
      ...
      seeds.push_back(Pifhr_seed{ c_2, field_i, c_0 / stride, c_1 / stride });
    }
  }
...
std::vector<OpenPifPafPostprocessor::frontier_t> OpenPifPafPostprocessor::frontier(Annotation& ann) {
  ...
  for (int connection_i = 0; connection_i < numBones; ++connection_i) {
    ...
    if (ann.keypoints[j1i*3 + 2] > 0.0f && ann.keypoints[j2i*3 + 2] == 0.0f) {
      f.push_back(frontier_t{ ann.keypoints[j1i*3 + 2], connection_i, true, j1i, j2i });
    }
  }
  ...
  for (int connection_i = 0; connection_i < numBones; ++connection_i) {
    ...
    if (ann.keypoints[j2i*3 + 2] > 0.0f && ann.keypoints[j1i*3 + 2] == 0.0f) {
      f.push_back(frontier_t{ ann.keypoints[j2i*3 + 2], connection_i, false, j1i, j2i });
    }
  }
                                   ...
                                   float th)
{
  ...
  while (true) {
    ...
    // reverse match
    if (th >= 0.1) {
      const auto t = growConnection(new_x, new_y, directed_paf_field_reverse);
      const auto reverse_x = std::get<0>(t);
      const auto reverse_y = std::get<1>(t);
      const auto reverse_v = std::get<2>(t);
  ...
  const float* paf_b2)
{
  ...
  // TODO: output pif_b ("pif_spreads") does not appear to be used anywhere, so perhaps remove it from the model?
  ...
  */
  ...
  for (auto& ann : thresholded) {
    ...
    for (int k = 0; k < numKeypoints; ++k) {
      const int x = ann.keypoints[k*3    ] * tileCoordinates.size.x + tileCoordinates.origin.x;
      const int y = ann.keypoints[k*3 + 1] * tileCoordinates.size.y + tileCoordinates.origin.y;
      const auto v = ann.keypoints[k*3 + 2];
...
ai_app::Object_detection::Result OpenPifPafPostprocessor::postprocess_0_11(
  int inputWidth, int inputHeight, int tensorWidth, int tensorHeight,
  const ai_app::Rect& tileCoordinates,
  const float* cif,  // 19x9xHxW
  const float* caf  // 17x5xHxW
)
{
  ai_app::Object_detection::Result result;
